...
def dilate_shadow_mask(mask, kernel_size=0):
    """Dilate a binary shadow mask with an odd square kernel.

    ``kernel_size`` is the OpenCV dilation kernel size in pixels. ``0`` disables
    dilation. This is intended for visual/transition-region reporting; strict
    paper metrics should keep the original mask.
    """
    ...
    return cv2.dilate(binary, kernel, iterations=1).astype(np.float32)
...
def compute_paper_metrics(pred_rgb, gt_rgb, mask, size=256, include_lpips=True):
    """Compute SRD/HomoFormer-style metrics for one RGB image.

    DESOBA/SRD-style evaluation resizes estimated shadow-free images to
    256x256, reports RGB PSNR/SSIM, LAB-space MAE/RMSE, and optionally LPIPS.
    Region metrics use the shadow mask directly: white/nonzero is shadow,
    inverse is non-shadow. Empty regions return NaN.
    """
    ...
    if include_lpips:
        ...
        lpips_shadow = float("nan")
        lpips_nonshadow = float("nan")
        lpips_all = float("nan")
...
def _lpips_score(lpips_model, restored, target):
    if lpips_model is None:
        return float("nan")
    restored_t = torch.from_numpy(restored.transpose(2, 0, 1)).unsqueeze(0).float()
    target_t = torch.from_numpy(target.transpose(2, 0, 1)).unsqueeze(0).float()
    restored_t = restored_t * 2.0 - 1.0
    target_t = target_t * 2.0 - 1.0
    device = next(lpips_model.parameters()).device
    return float(lpips_model(restored_t.to(device), target_t.to(device)).item())


def compute_training_region_metrics(restored, target, mask, lpips_model=None):
    mask = mask.astype(bool)
    regions = {
        "Shadow": mask,
        "non-Shadow": ~mask,
        "all": np.ones(mask.shape, dtype=bool),
    }
    metrics = {}
    for name, region in regions.items():
        if not np.any(region):
            continue
        diff = restored[region] - target[region]
        mse = float(np.mean(diff ** 2))
        psnr = 100.0 if mse <= 1.0e-12 else float(10.0 * np.log10(1.0 / mse))
        mae = float(np.mean(np.abs(diff)) * 255.0)
        rmse = float(np.sqrt(np.mean((diff * 255.0) ** 2)))

        isolated = target.copy()
        isolated[region] = restored[region]
        crop = _metric_crop(region)
        if crop is None:
            ssim = float("nan")
            ...
            win_size = min(7, crop_h, crop_w)
            if win_size % 2 == 0:
                win_size -= 1
            if win_size >= 3:
                ssim = float(ssim_loss(
                    target[y0:y1, x0:x1], isolated[y0:y1, x0:x1],
                    channel_axis=2, data_range=1.0, win_size=win_size))
            else:
                ssim = float("nan")
            lpips_value = _lpips_score(lpips_model, isolated, target)
        metrics[name] = {
            "PSNR": psnr, "SSIM": ssim, "LPIPS": lpips_value,
            "MAE": mae, "RMSE": rmse,
        }
    return metrics
